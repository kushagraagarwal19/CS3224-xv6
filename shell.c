...
runcmd(struct cmd *cmd)
{
	...
	case ' ':
		...
		fprintf(stderr, "Some error while running command \"%s\"\n", ecmd->argv[0]);
		// Your code here ...
		break;
	case '>':
	case '<':
	...
	case '|':
		...
		//Process1 - this will read from pipe
		if(p[0] == 0) {
			if(dup2(pipefd[0],0)< 0) {
				fprintf(stderr, "Can't duplicate File Descriptor\n");
				exit(0);
			}
			close(pipefd[1]); //Close the unused write
			runcmd(pcmd->right);
		}
		//Child process - this will read from pipe
		if(p[1] == 0) {
			if(dup2(pipefd[1],1)< 0) {
				fprintf(stderr, "Can't duplicate File Descriptor\n");
				exit(0);
			}
			close(pipefd[0]); //Close the unused read
			runcmd(pcmd->left);
		}
		close(pipefd[0]);
		close(pipefd[1]);
